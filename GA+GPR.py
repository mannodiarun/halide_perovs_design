...
##  ML Model is fixed to be Random Forest Regression (gpr) here  ##
...
gpreg_decomp = GridSearchCV(GaussianProcessRegressor(normalize_y=False), param_grid=param_grid, cv=5)
gpreg_decomp.fit(X_train_fl, Prop_decomp_train_fl)
Pred_train = gpreg_decomp.predict(X_train_fl)
Pred_test = gpreg_decomp.predict(X_test_fl)
Pred_out = gpreg_decomp.predict(X_out_fl)
...
gpreg_gap = GridSearchCV(GaussianProcessRegressor(normalize_y=False), param_grid=param_grid, cv=5)
gpreg_gap.fit(X_train_fl, Prop_gap_train_fl)
Pred_train = gpreg_gap.predict(X_train_fl)
Pred_test = gpreg_gap.predict(X_test_fl)
Pred_out = gpreg_gap.predict(X_out_fl)
...
if Data == 'PBE':
    gpreg_fom = GridSearchCV(GaussianProcessRegressor(normalize_y=False), param_grid=param_grid, cv=5)
    gpreg_fom.fit(X_train_fl, Prop_fom_train_fl)
    Pred_train = gpreg_fom.predict(X_train_fl)
    Pred_test = gpreg_fom.predict(X_test_fl)
    Pred_out = gpreg_fom.predict(X_out_fl)
...
def f(XX):
    ...
    for i in range(11,14):
        X[i] = x2[int(XX[i])]
    pen = 0
    if np.abs(np.sum(X[0:5]) - 1.0) > 0.01:
        pen = pen + 1000*np.abs(1-np.sum(X[0:5]))
    if np.abs(np.sum(X[5:11]) - 1.0) > 0.01:
        pen = pen + 1000*np.abs(1-np.sum(X[5:11]))
    if np.abs(np.sum(X[11:14]) - 3.0) > 0.01:
        pen = pen + 1000*np.abs(3-np.sum(X[11:14]))
    pred_decomp = float(gpreg_decomp.predict( np.reshape(X, (1, -1)) ))
    pred_gap = float(gpreg_gap.predict( np.reshape(X, (1, -1)) ))
    if Data == 'PBE':
        pred_fom = -1*float(gpreg_fom.predict( np.reshape(X, (1, -1)) ))
        if pred_gap < 1.0:
...
print ('best_decomp_energy = ', float ( gpreg_decomp.predict( np.reshape(X_best[:], (1,-1)) ) ))
print ('best_band_gap = ', float ( gpreg_gap.predict( np.reshape(X_best[:], (1,-1)) ) ))
if Data == 'PBE':
    print ('best_PV_FOM = ', float ( gpreg_fom.predict( np.reshape(X_best[:], (1,-1)) ) ))

#print ('best_decomp_energy = ', float(gpreg_decomp.predict( np.reshape(model.best_variable, (1, -1)) )) )
#print ('best_PV_FOM = ', float(gpreg_fom.predict( np.reshape(model.best_variable, (1, -1)) )) )
#print ('best_band_gap = ', float(gpreg_gap.predict( np.reshape(model.best_variable, (1, -1)) )) )
...
text_file.write(":: %s" % '  ' + '\n')
text_file.write("best_decomp_energy: %s" % float(gpreg_decomp.predict( np.reshape(X_best[:], (1, -1)) )) + ' eV' + '\n')
text_file.write("best_band_gap: %s" % float(gpreg_gap.predict( np.reshape(X_best[:], (1, -1)) )) + ' eV' + '\n')
if Data == 'PBE':
    text_file.write("best_PV_FOM: %s" % float(gpreg_fom.predict( np.reshape(X_best[:], (1, -1)) )) + '\n')
